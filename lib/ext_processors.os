...
Процедура ВыполнитьСборкуОбработок() Экспорт
	...
	ШаблонИмениОбработки = ОбъединитьПути(мКаталогОбработок, "proc2", "%1");
...
Процедура ВыполнитьЗагрузкуОбработокВИнформационнуюБазу(КаталогОбработок) Экспорт
	
	ПараметрыОбработки = ПараметрыЗапускаВнешнейОбработки();
	ПараметрыОбработки.РежимРаботы = "Загрузка";
	ПараметрыОбработки.КаталогОбработок = СтрШаблон("""%1""", КаталогОбработок);
	ПараметрыОбработки.ПутьКЛогам = мЖурналы.ЖурналЗагрузкиОбработок;
	КлючЗапуска = КлючЗапускаВнешнейОбработки(ПараметрыОбработки);
	
	мКонфигуратор.УстановитьИмяФайлаСообщенийПлатформы(мЖурналы.ЖурналЗагрузкиОбработок, Ложь);
...
Процедура ВыполнитьВыгрузкуОбработокИзИнформационнойБазы() Экспорт
	...
	мКонфигуратор.УстановитьИмяФайлаСообщенийПлатформы(мЖурналы.ЖурналВыгрузкиОбработок, Ложь);

	// Сообщить(КлючЗапуска);
	ВыполнениеОбработки = СтрШаблон("/execute""%1""", мИмяОбработкиВыгрузкиЗагрузки);
	Попытка
		мКонфигуратор.ЗапуститьВРежимеПредприятия(КлючЗапуска, , ВыполнениеОбработки);
...
Функция ПараметрыЗапускаВнешнейОбработки()
	...
	// Параметры.Вставить("ExecuteADP");
	// Параметры.Вставить("name");
	// Параметры.Вставить("command");
	// Параметры.Вставить("name");
...
// ПараметрыЗапуска.Добавить(СтрШаблон("/AgentBaseDir ""%1""", "D:\Users\AAZ\Задачи\ERP\51542"));
// ПараметрыЗапуска.Добавить("/Visible");

// Конфигуратор.ВыполнитьКоманду(ПараметрыЗапуска);

// КлиентSSH = Новый КлиентSSH("127.0.0.1", 1543, "defuser", "defuser");
...
// Результат = Поток.ЗаписатьВПоток(СтрШаблон("config dump-ext-files --file ""%1"" --ext-file ""%2""", 
// 			".\Сборка\0a1fd53c-c39b-11e7-81dd-00505601004a.epf", ".\Сборка\0a1fd53c-c39b-11e7-81dd-00505601004a"));
