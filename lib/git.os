...
Функция ОбработатьФайлИзменений(ПутьКРепозитарию, ИмяФайлаСпискаФайлов, Журнал) Экспорт
	...
	Пока не Стр = неопределено Цикл
		...
		НЕ (СтрНайти(СтрЗТ, "ObjectModule") > 0 ИЛИ СтрНайти(СтрЗТ, "ManagerModule") > 0 ИЛИ СтрНайти(СтрЗТ, "CommandModule") > 0) // Эти модуля загружаются частично из папки \Ext объектов
