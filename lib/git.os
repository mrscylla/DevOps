...
Функция ОбработатьСтрокуИзмененияВнешнихОбработок(Знач Стр, Знач СтрЗТ, ПутьКРепозиторию, Журнал)
	...
	прФайл = Неопределено;
	НайденКаталогОбработки = Ложь;
	Пока Не НайденКаталогОбработки Цикл
		Файл = Новый Файл(СтрЗТ);
		Если Не прФайл = Неопределено И Файл.ПолноеИмя = прФайл.ПолноеИмя Тогда
			СтрЗТ = Неопределено;
			Прервать;
		КонецЕсли;

		прФайл = Файл;
		Если Не СтроковыеФункции.ЭтоУникальныйИдентификатор(Файл.Имя) Тогда
			СтрЗТ = Файл.Путь;
			БылоПреобразование = Истина;
		Иначе
			НайденКаталогОбработки = Истина;
		КонецЕсли;
...
Функция ТаблицаИзмененныхФайлов(ПутьКРепозиторию, ОтслеживаемыеИзменения, ИмяФайлаСпискаФайлов, Журнал) Экспорт
	...
	Пока Не Стр = Неопределено Цикл
		...
		Если Не Стр[0] = "D" Или СтрНайти(ИзмененныйФайл, РежимыОтслеживаемыхИзменений.ВнешниеОбработки) Тогда
			...
			СтрТз.Уровень = СтрЧислоВхождений(ИзмененныйФайл, "\") * ?(ЭтоНовыйОбъект, -1, 1);
