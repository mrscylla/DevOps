...
Функция ОбработатьФайлИзменений(ПутьКРепозитарию, ИмяФайлаСпискаФайлов, Журнал) Экспорт
	...
	Пока не Стр = неопределено Цикл
		...
		Если Не СокрЛП(Стр) = "" И НЕ Стр[0] = "R094" Тогда

			СтрЗТ = "" + ПутьКРепозитарию + "\" + СтрЗаменить(Стр[1],"/","\");
			
		ИначеЕсли Стр[0] = "R094" Тогда
	...
	// убираем из списка все добавленные ("A") объекты, они будут загружаться при загрузке их родительских объектов
	МасУд = Новый Массив;
	Для каждого СтрТз из ТЗ Цикл

		Если СтрТз.Новый Тогда

			МасУд.Добавить(СтрТз);

		КонецЕсли;	

	КонецЦикла;	

	Для каждого СтрУд Из МасУд Цикл
