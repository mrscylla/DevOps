...
Процедура ВыполнитьЗагрузкуИзмененийОсновнойКонфигурации()
	...
	Если Результат Тогда												
		Сообщение = СтрШаблон("%1 загрузка из файлов успешно завершена (см. %3)", 
													?(ЧастичнаяЗагрузка, "Частичная", "Полная"),
													Журналы.ЖурналЗагрузкиИзменений);
	Иначе
		РезультатыКоманд.ЗагрузкаИзменений = Ложь;
		Сообщение = СтрШаблон("%1 загрузка из файлов не удалась (см. %3)", 
													?(ЧастичнаяЗагрузка, "Частичная", "Полная"),
					 								Журналы.ЖурналЗагрузкиИзменений);
	КонецЕсли;
