...
Если ЗагрузитьИзменения ИЛИ ЗагрузитьВсе Тогда 
	...
	Иначе
		...
		Если КолВоИзменений <> 0 Тогда
			...
			Журнал.Информация("Запуск: """ + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь + """ /P ""******"" /LoadConfigFromFiles " + ПутьВременногоКаталогаЗагрузки + "\Config" + ?(ОбновитьДампКонфигурации, " -updateConfigDumpInfo", "") + ?(ЧастичнаяЗагрузка, " -ListFile " + ИмяФайлаСпискаФайлов, "") + ?(ОбновитьБазуДанных, " /UpdateDBCfg", "") + " /out " + ОбъединитьПути(КаталогЖурналов, "loadchanges_" + ИмяБазы + ".log"));
			ПроцессКонфигуратора = Создатьпроцесс("""" + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь +""" /P """ + Пароль + """ /LoadConfigFromFiles " + ПутьВременногоКаталогаЗагрузки + "\Config" + ?(ОбновитьДампКонфигурации, " -updateConfigDumpInfo", "") + ?(ЧастичнаяЗагрузка, " -ListFile " + ИмяФайлаСпискаФайлов, "") + ?(ОбновитьБазуДанных, " /UpdateDBCfg", "") + " /out " + ЖурналЗагрузкиИзменений
													,ПутьКРепозитарию
													,Истина
													,Ложь
