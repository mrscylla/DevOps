...
Если ЗагрузитьИзменения ИЛИ ЗагрузитьВсе Тогда 
	...
	Иначе
		...
		Если КолВоИзменений <> 0 Тогда
			...
			ПроцессКонфигуратора = Создатьпроцесс("""" + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь +""" /P """ + Пароль + """ /LoadConfigFromFiles " + ПутьКРепозитарию + "\Config" + ?(ОбновитьДампКонфигурации, " -updateConfigDumpInfo", "") + ?(ЧастичнаяЗагрузка, " -ListFile " + ИмяФайлаСпискаФайлов, "") + ?(ОбновитьБазуДанных, " /UpdateDBCfg", "") + " /out " + ЖурналЗагрузкиИзменений
													,ПутьКРепозитарию
													,Истина
													,Ложь
			...
			ИзмененияЗагруженыУспешно = СтрНайти(РезультатЗагрузкиИзменений, "Обновление конфигурации успешно завершено") > 0 ИЛИ НЕ ОбновитьБазуДанных;
			ЧТ.Закрыть();	
