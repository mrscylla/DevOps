...
Если КолВоИзменений <> 0 Тогда
	...
	Журнал.Информация("Запуск: """ + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь + """ /P ""******"" /LoadConfigFromFiles " + ПутьКРепозитарию + "\Extension -Extension ""Расширение_РТИТС"" -updateConfigDumpInfo /UpdateDBCfg /out " + ЖурналЗагрузкиИзмененийРасширения);
	ПроцессКонфигуратора = Создатьпроцесс("""" + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь +""" /P """ + Пароль + """ /LoadConfigFromFiles " + ПутьКРепозитарию + "\Extension -Extension ""Расширение_РТИТС"" -updateConfigDumpInfo /UpdateDBCfg /out " + ЖурналЗагрузкиИзмененийРасширения
											,ПутьКРепозитарию
											,Истина
											,Ложь
											,КодировкаТекста.UTF8);
