...
Если ЗагрузитьИзменения ИЛИ ЗагрузитьВсе Тогда 
	...
	Иначе
		...
		Если КолВоИзменений <> 0 Тогда
			...
			Журнал.Информация("Запуск: """ + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь + """ /P ""******"" /LoadConfigFromFiles " + ПутьКРепозитарию + "\Config" + ?(ОбновитьДампКонфигурации, " -updateConfigDumpInfo", "") + ?(ЧастичнаяЗагрузка, " -ListFile " + ИмяФайлаСпискаФайлов, "") + ?(ОбновитьБазуДанных, " /UpdateDBCfg", "") + " /out " + ОбъединитьПути(КаталогЖурналов, "loadchanges_" + ИмяБазы + ".log"));
			ПроцессКонфигуратора = Создатьпроцесс("""" + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь +""" /P """ + Пароль + """ /LoadConfigFromFiles " + ПутьКРепозитарию + "\Config" + ?(ОбновитьДампКонфигурации, " -updateConfigDumpInfo", "") + ?(ЧастичнаяЗагрузка, " -ListFile " + ИмяФайлаСпискаФайлов, "") + ?(ОбновитьБазуДанных, " /UpdateDBCfg", "") + " /UpdateDBCfg /out " + ЖурналЗагрузкиИзменений
													,ПутьКРепозитарию
													,Истина
													,Ложь
...
Если ВыгружатьРасширение Тогда
	...
	Иначе
		...
		ПроцессКонфигуратора = Создатьпроцесс("""" + ПутьКПлатформе1С + """ DESIGNER /UC 456654 /S """ + ИмяСервера + "\" + ИмяБазы + """ /N """ + Пользователь +""" /P """ + Пароль + """ /DumpCfg -Extension ""Расширение_РТИТС"" " + ОбъединитьПути(ПутьКПоставке, ИмяБазы + "_ext.cf") + " /out " + ЖурналВыгрузкиФайлапоставкиРасширения
												,ПутьКРепозитарию
												,Истина
												,Ложь
												,КодировкаТекста.UTF8);
