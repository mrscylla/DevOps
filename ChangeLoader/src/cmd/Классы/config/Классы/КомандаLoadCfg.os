...
Процедура ПрочитатьВходящиеПараметры(Знач Команда)
	...
	ЛогПриложения.Информация(СтрШаблон("Каталог репозитария: %1", КаталогРепозитория));
...
Функция ПодготовитьНастройкиЗагрузкиКонфигураций()
	...
	Настройки.Колонки.Добавить("ОчищатьКаталогЗагрузки");
...
Процедура ДобавитьНастройкуЗагрузкиКонфигурации(Знач Настройки, Знач Наименование, Знач Каталог, Знач ДополнительныйКлюч = "", ТолькоИзменения = Ложь)
	...
	НоваяСтрока.ОчищатьКаталогЗагрузки = ТолькоИзменения; 
	НоваяСтрока.КаталогДляЗагрузки = ?(ТолькоИзменения, ВременныйКаталогДляЗагрузки(), Каталог);
...
Процедура ВыполнитьЗагрузкуКонфигураций(Знач НастройкиЗагрузки)
	...
	Для Каждого Настройка Из НастройкиЗагрузки Цикл
		...
		КаталогДляЗагрузки = Настройка.КаталогДляЗагрузки;
		...
		Попытка
			Информация = СтрШаблон(
				"Запуск конфигуратора для загрузки
				|%1", СтрСоединить(ПараметрыЗапуска, " "));
			ЛогПриложения.Информация(Информация);
