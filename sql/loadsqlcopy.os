...
Процедура ПрочитатьНастройкиБазДанных(ИмяПлана)
	...
	Для Каждого Элемент Из МассивПараметровПланов Цикл
		Если ИмяПлана = Элемент["baseType"] Тогда
			ПараметрыЗадания.Вставить("ТипБазы", 				Элемент["baseType"]); // erpnm, erp, ecm
			ПараметрыЗадания.Вставить("БазаИсточник", 			Элемент["source_ibname"]); // erp_nm_Production
			ПараметрыЗадания.Вставить("БазаПриемникШаблон", 	Элемент["dest_ibname"]); // erp_nm, erp, ecm
	...
	ПараметрыЗадания.Вставить("ИмяПользователя", СтрЗаменить(ПараметрыЗадания["БазаПриемник"], ПодстрокаЗамены, "")); // Для получения файла логов ERPNM_AAZ -> AAZ
	ПараметрыЗадания["БазаПриемник"] = СтрШаблон("%1_%2", ПараметрыЗадания["БазаПриемникШаблон"], ПараметрыЗадания["ИмяПользователя"]);
...
Процедура ОтправитьПочтовоеСообщение(Знач АдресПолучателя, ТекстСообщения)
	...
	Адрес = Сообщение.Получатели.Добавить(АдресПолучателя);
...
Процедура РазобратьВходящиеПараметры()
	...
	// Примеры: erp-aaz-1185, erp-aaz-03-1185, erpnm-aaz-1185
	...
	ТипБазы = СтрРазделить(ПараметрыЗадания["БазаПриемник"], "_");
	ТипБазы.Удалить(ТипБазы.Количество() - 1);
	ТипБазы = СтрСоединить(ТипБазы, "_");
	
	ПрочитатьНастройкиБазДанных(ТипБазы);
...
Процедура ВосстановитьБазуИзАрхива()
	...
	ИсполняемыйФайл = "C:\Program Files\Microsoft SQL Server\Client SDK\ODBC\130\Tools\Binn\SQLCMD.EXE";
	ФайлЛога = СтрШаблон("%1\Logs\sql.log", ПараметрыЗадания["РабочийКаталог"]);
	Команда = СтрШаблон("""%1"" -S %2 -v sourcedb=""%3"" db=""%4"" bakfile=""%5"" owner=""%6"" -i %7 -o ""%8""",
		ИсполняемыйФайл,
